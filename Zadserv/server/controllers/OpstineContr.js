		...
		if (uid) {
			...
			var slog = sfDrzava.findById(Drzava, (err, doc) => {
				if (err) {
					return res.status(400).json({
						success: false,
						message: 'Error Drzava not exist ' + err,
						data: []
					});
				}
				...
				oOpstina.save(function (err, result) {
					...
					Opstina.find({
						_id: oOpstina._id
					}).populate('Drzava', ['KodDrzave', 'Naziv']).exec(function (err, result) {
						if (err) {
							return res.status(400).json({
								success: false,
								message: 'Error processing request ' + err,
...
module.exports.listopstine = function (req, res, next) {
	...
	Opstina.find({}).sort({
		created_at: -1
	}).populate('Drzava', ['Naziv', 'KodDrzave']).exec(function (err, result) {
		if (err) {
			return res.status(400).json({
				success: false,
				message: 'Error processing request ' + err,
...
module.exports.pickopstine = function (req, res, next) {
	...
	Opstina.find({}).sort({
		Naziv: 1
	}).select('RegOzn Naziv DrzavaRef').exec(function (err, result) {
		if (err) {
			return res.status(400).json({
				success: false,
				message: 'Error processing request ' + err,
