...
module.exports.listpartner = function (req, res, next) {
	...
	Partner.find({}).sort({
		created_at: -1
	}).populate('Drzava', ['Naziv']).exec(function (err, result) {
		if (err) {
			res.statusMessage = err;
			return res.status(400).json({
				success: false,
...
module.exports.pickactivpartner = function (req, res, next) {
	...
	Partner.find({
		...
		Naziv: 1
	}).select('KyeSearch Naziv DrzavaRef Adresa.AdMesto Pib').exec(function (err, result) {
		if (err) {
			res.statusMessage = err;
			return res.status(400).json({
				success: false,
...
module.exports.countkupacActiv = function (req, res, next) {
	//console.log("parametar je : " + req.params.id);
	Partner.count({
		Aktivan: true
	}, function (err, count) {
