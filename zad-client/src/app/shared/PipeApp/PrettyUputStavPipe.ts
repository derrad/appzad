...
@Pipe({
  ...
  export class PrettyUputStavPipe implements PipeTransform {
   ...
   private GetTable(tUputStav: UputStavNew[]): string {
        ...
        html += '<th class="col-md-6"><p class="text-left">Zadrugar</p></th>';
        html += '<th class="col-md-2"><p class="text-center">TipZad</p></th>';
        html += '<th class="col-md-2"><p class="text-left">Posao</p></th>';
        html += '</tr>';
        html += '</thead>';
        html += '<tbody>';
        tUputStav.forEach(element => {
            ...
            html += '<td class="col-md-6">' + element.ZadRef.name + '</td>';
            html += '<td class="col-md-2">  <p class="text-center">' + element.ZadRef.tipzadrugar + '</p></td>';
            html += '<td class="col-md-2">' + element.PosloviRef.name + '</td>';
            html += '</tr>';
        });
        html += '</tbody>';
        html += '</table>';
